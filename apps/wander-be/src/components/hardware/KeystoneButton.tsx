...
export default function KeystoneButton({ onSuccess }: Props) {
  ...
  return (
    <>
      <Button fullWidth variant="secondary" onClick={() => connectModal.setOpen(true)} style={{ gap: "3px" }}>
        <KeystoneIcon size={16} />
        {browser.i18n.getMessage("keystone_connect_title")}
      </Button>
      <QRModal {...connectModal.bindings} root={document.getElementById("__plasmo")}>
        <ModalText heading>{browser.i18n.getMessage("keystone_connect_title")}</ModalText>
        <ModalContent>
          <AnimatedQRScanner
            {...scanner.bindings}
            onError={(error) =>
          ...
          <Spacer y={1} />
          <CancelButton onClick={cancel}>{browser.i18n.getMessage("cancel")}</CancelButton>
        </ModalContent>
      </QRModal>
    </>
  );
