 ...
 */
export function signAuth(appData: ModuleAppData, transaction: Transaction, address: string) {
  log(LOG_GROUP.AUTH, "signAuth()", transaction);

  return new Promise<AuthSuccessResult<SignatureResult | undefined>>(async (resolve, reject) => {
    // generate chunks
    const {
      transaction: tx,
      dataChunks,
    ...
    try {
      log(
        LOG_GROUP.CHUNKS,
        `Sending ${dataChunks.concat(tagChunks).length || 0} txs chunks for collection`,
        collectionID,
        "to popup",
        popupWindowTabID,
      );
      ...
      log(LOG_GROUP.CHUNKS, `Sending "end" chunk for collection`, collectionID, "to popup", popupWindowTabID);
      ...
      log(LOG_GROUP.CHUNKS, "Done sending txs chunks for collection", collectionID, "to popup", popupWindowTabID);
    } catch (err) {
      return reject(`Error in signAuth while sending a data chunk of collection "${collectionID}": \n${err}`);
    }
  });
}
...
export function signAuthKeystone(appData: ModuleAppData, dataToSign: AuthKeystoneData) {
  ...
  return new Promise<AuthSuccessResult<SignatureResult | undefined>>(async (resolve, reject) => {
    ...
    try {
      ...
      return reject(`Error in signAuthKeystone while sending a data chunk of collection "${collectionID}": \n${err}`);
    }
  });
}
