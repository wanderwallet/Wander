...
export function AccountBackupWalletRecoveryFileEmbeddedView() {
  ...
  const handleGenerateRecoveryAndDownload = useCallback(async () => {
    try {
      ...
      toast.error("Error generating recovery file");
    } finally {
      setIsLoading(false);
    }
    ...
    <Card
      ...
      <Box style={{ gap: 28 }}>
        <Flex
          ...
          <Text variant="bodyMd" alignment="left" style={{ color: "var(--text-color-primary)" }}>
            About this file:
          </Text>
          <Flex direction="row" gap={8} align="center">
            <XClose style={{ flexShrink: 0 }} height={17} width={17} color="var(--brand-color-error2)" />
            <Text variant="bodyXs" alignment="left" style={{ color: "var(--text-color-primary)" }}>
