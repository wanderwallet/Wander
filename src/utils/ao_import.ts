...
export const AO_NATIVE_TOKEN_BALANCE_MIRROR = "Pi-WmAQp2-mh-oWH9lWpz5EthlUDj_W0IusAv-RXhRk";
// export const AO_NATIVE_OLD_TOKEN =
//   "BJj8sNao3XPqsoJnea4DnJyPzHnKhkhcY1HtWBxHcLs";
export const AO_NATIVE_OLD_TOKEN = "m3PaWzK4PTG9lAaqYQPaPdOcXdO8hYqi5Fe9NWqXd0w";
...
export async function updateAoToken() {
  try {
    ...
    const updatedAoTokens = aoTokens.filter((token) => token.processId !== AO_NATIVE_OLD_TOKEN);
    const isAoTokenPresent = updatedAoTokens.some((token) => token.processId === AO_NATIVE_TOKEN);
    if (!isAoTokenPresent) {
